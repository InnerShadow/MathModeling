...
task3_3 <- function(vec){
  ...
  for(i in 1 : length(vec)){
    omegasqr <- omegasqr + (probobility(vec[i]) - (i - .5) / length(vec)) ^ 2
  }
  cat("counted omega^2:", omegasqr, "table 95% chi^2:", 0.46, 
      "\nis counted < critical p-value?", (omegasqr < .46))
}

dens <- function(x) 1.5 * x ^ (1 / 2)

probobility <- function(x) x ^ (3 / 2)

l <- function(x) 1 / G + 0 * x

G <- max(dens(seq(0, 1, length = 10000)))

r <- G / (integrate((function(x) (G + x * 0)), 0, 1)$value) # x' = runif(0, 1)
                                                            # y' = 3 / 2 * runif(0, 1)
N <- 10000
i <- 1
